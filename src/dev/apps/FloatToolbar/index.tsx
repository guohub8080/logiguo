...
import {bgColorAtom, viewPaddingAtom, articleScrollAtom, maxScrollAtom} from "../../store/useViewSettingsStore.ts";

interface FloatToolbarProps {
  clearQueryCompare: () => void;
  switchSide: () => void;
}
...
const FloatToolbar = (props: FloatToolbarProps) => {
  ...
  const maxScroll = useAtomValue(maxScrollAtom);
  const setBgColor = useSetAtom(bgColorAtom);
  const setViewPadding = useSetAtom(viewPaddingAtom);
  const setArticleScroll = useSetAtom(articleScrollAtom);
