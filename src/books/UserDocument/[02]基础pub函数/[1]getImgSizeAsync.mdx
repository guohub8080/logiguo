...
// 使用 es-toolkit 的 isNil 检查 null 或 undefined
...
**代码亮点**：
- 使用 es-toolkit 的 `isNil()` 函数同时检查 `null` 和 `undefined`
- 简洁高效，语义清晰
- 避免了复杂的逻辑判断，代码更易读
...
- [`isNil`](https://es-toolkit.slash.me/reference/predicate/isNil.html) - es-toolkit 空值检查函数（检查 null 或 undefined）
