...
2. **事件触发**：设置 `begin="click"` 或库参数 `beginType: 'click'`，点击后重播
...
// 方式2：点击触发
{genAnimateScale({ timeline: [...], beginType: 'click' })}
...
- **点击触发**：使用 `beginType: 'click'`，用户点击目标元素即可播放/重播
...
// 或 2）点击触发（无需 id）
{genAnimateScale({ timeline: [...], beginType: 'click', loopCount: 1 })}
```
