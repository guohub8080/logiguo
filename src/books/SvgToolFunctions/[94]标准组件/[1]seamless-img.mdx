export const title = "无缝图组件 SeamlessImg"
...
- **尺寸参数**：虽然 w 和 h 是可选的，但建议明确提供，避免自动获取失败
- **URL 格式**：建议使用 HTTPS 协议的图片地址
- **深色对抗**：如果不需要深色对抗，使用 `isNaturalPriority={true}`
- **性能考虑**：大量使用 foreignObject 可能影响性能，优先使用默认模式
- **兼容性**：所有模式在微信公众号中测试通过
- **嵌套限制**：如果需要在 SVG 内嵌入 HTML 元素，必须使用 foreignObject
- **命名空间**：使用 foreignObject 时记得声明 xmlns 和 xmlnsXlink
